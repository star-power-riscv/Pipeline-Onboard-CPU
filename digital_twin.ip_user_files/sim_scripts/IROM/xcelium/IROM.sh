...
#
# Script generated by Vivado on Thu Jun 19 13:23:12 +0800 2025
...
#
# Usage        : IROM.sh
#                IROM.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                IROM.sh [-reset_run]
#                IROM.sh [-reset_log]
#                IROM.sh [-help]
#
...
#                script (type 'IROM.sh -help' for more information).
#
...
# script info
echo -e "IROM.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./IROM.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xmelab $xmelab_opts $design_libs_elab xil_defaultlib.IROM xil_defaultlib.glbl
}

# RUN_STEP: <simulate>
simulate()
{
  xmsim $xmsim_opts xil_defaultlib.IROM -input simulate.do
}
...
reset_run()
{
  files_to_remove=(xmvlog.log xmvhdl.log xmsc.log compile.log elaborate.log simulate.log diag_report.log xsc_report.log IROM_sc.so .tmp_log xcelium_lib waves.shm c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./IROM.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: IROM.sh [-help]\n\
Usage: IROM.sh [-step]\n\
Usage: IROM.sh [-lib_map_path]\n\
Usage: IROM.sh [-reset_run]\n\
Usage: IROM.sh [-reset_log]\n\
Usage: IROM.sh [-keep_index]\n\
Usage: IROM.sh [-noclean_files]\n\n\
